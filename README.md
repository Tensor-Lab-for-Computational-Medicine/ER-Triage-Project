...
Set `ED_SIM_CASE_DIR` to point the backend at a local prepared-case directory. The loader accepts either legacy flat `*.json` files or portable case bundle folders. A bundle folder must contain a playable `prepared_case.json`; it may also include `case_bundle.json` and local-only artifacts such as source probes, readiness reports, playthrough proof, and clinician review templates. The Playwright config sets this to an empty fixture path by default so e2e tests use the deterministic sample case instead of whichever restricted cases are present in `data/cases/`.
...
data/cases/
  local_case_id/
    prepared_case.json          # required playable PreparedCase
    case_bundle.json            # local manifest for humans/tools
    artifacts/                  # optional local-only audit/source/review files
```

To share one restricted case with an approved collaborator, zip the specific case folder only. The recipient can unzip it anywhere and run the backend with `ED_SIM_CASE_DIR` pointing either to the folder itself or to the parent directory that contains multiple case folders. Do not commit the folder or zip; `data/cases/` is ignored because these artifacts are derived from credentialed PhysioNet/MIMIC data.

Set `ED_SIM_DEFAULT_CASE_ID` when a deployment should open one specific local case by default. For direct QA links, `/ai-simulator?case_id=local_case_id` starts that case explicitly if the backend loaded it from `ED_SIM_CASE_DIR`.
...
python -m backend.cases.mimic_ext data/restricted/mietic_mimic_main_ed_enriched_cases.restricted.json `
  --case-id local_case_id `
  --cxr-reports-dir D:\Downloads\mimic-cxr-reports `
  --output data/cases/local_case_id/prepared_case.json
```
...
python -m backend.cases.mimic_ext data/restricted/mietic_mimic_main_ed_enriched_cases.restricted.json `
  --case-id local_case_id `
  --supplemental-results data/cases/local_case_id/artifacts/local_case_id.results.local.json `
  --output data/cases/local_case_id/prepared_case.json
```
...
python -m backend.cases.case_pool_audit data/restricted/mietic_mimic_main_ed_enriched_cases.restricted.json `
  --selected-case-id local_case_id `
  --selected-supplemental-results data/cases/local_case_id/artifacts/local_case_id.results.local.applied.json `
  --source-root D:\physionet `
  --ecg-index-report data/cases/local_case_id/artifacts/local_case_id.ecg-index.local.json `
  --top-n 50 `
  --output data/cases/local_case_id/artifacts/local_case_id.case-pool-audit.local.json
```
...
```powershell
python -m backend.cases.pilot_readiness_bundle data/cases/local_case_id/prepared_case.json `
  --playthrough-script data/cases/local_case_id/artifacts/local_case_id.playthrough.local.json `
  --source-probe-report data/cases/local_case_id/artifacts/local_case_id.source-probe.local.json `
  --source-acquisition-report data/cases/local_case_id/artifacts/local_case_id.source-acquisition.local.json `
  --source-acquisition-preflight-report data/cases/local_case_id/artifacts/local_case_id.source-acquisition-preflight.local.json `
  --case-pool-audit-report data/cases/local_case_id/artifacts/local_case_id.case-pool-audit.local.json `
  --output data/cases/local_case_id/artifacts/local_case_id.pilot-readiness.json `
  --artifact-dir data/cases/local_case_id/artifacts/local_case_id-readiness
```
...
```powershell
python -m backend.cases.case_pivot_plan data/cases/local_case_id/artifacts/local_case_id.case-pool-audit.local.json `
  --source-refresh-report data/restricted/local_case_id.source-refresh.local.json `
  --source-root D:\physionet `
  --enriched-source-file data/restricted/mietic_mimic_main_ed_enriched_cases.restricted.json `
  --output data/restricted/local_case_id.pivot-plan.local.json
```
...
```powershell
python -m backend.cases.clinician_review_dossier data/cases/local_case_id/prepared_case.json `
  --playthrough-script data/cases/local_case_id/artifacts/local_case_id.playthrough.local.json `
  --source-probe-report data/cases/local_case_id/artifacts/local_case_id.source-probe.local.json `
  --source-acquisition-report data/cases/local_case_id/artifacts/local_case_id.source-acquisition.local.json `
  --source-acquisition-preflight-report data/cases/local_case_id/artifacts/local_case_id.source-acquisition-preflight.local.json `
  --package data/restricted/heldout-validation-packages/local_case_id.package.json `
  --package data/restricted/heldout-validation-packages/local_case_id.package.json `
  --package data/restricted/heldout-validation-packages/local_case_id.package.json `
  --output data/cases/local_case_id/artifacts/local_case_id.clinician-review-dossier.local.json `
  --review-template-output data/cases/local_case_id/artifacts/local_case_id.review.local.template.json
```
...
```powershell
python -m backend.cases.goal_audit data/cases/local_case_id/prepared_case.json `
  --playthrough-script data/cases/local_case_id/artifacts/local_case_id.playthrough.local.json `
  --source-acquisition-report data/cases/local_case_id/artifacts/local_case_id.source-acquisition.local.json `
  --source-acquisition-preflight-report data/cases/local_case_id/artifacts/local_case_id.source-acquisition-preflight.local.json `
  --output data/cases/local_case_id/artifacts/local_case_id.goal-audit.json
```
...
```powershell
python -m backend.cases.source_gaps data/cases/local_case_id/prepared_case.json `
  --output data/cases/local_case_id/artifacts/local_case_id.source-gaps.json `
  --template-output data/cases/local_case_id/artifacts/local_case_id.results.local.template.json
```
...
python -m backend.cases.source_ecg_index data/restricted/mietic_mimic_main_ed_enriched_cases.restricted.json `
  --case-id local_case_id `
  --mimic-ecg-dir "D:\physionet\mimic-iv-ecg-diagnostic-electrocardiogram-matched-subset-1.0.zip" `
  --limit-per-subject 10 `
  --output data/cases/local_case_id/artifacts/local_case_id.ecg-index.local.json
```
...
```powershell
python -m backend.cases.source_probe data/cases/local_case_id/prepared_case.json `
  --source-root D:\physionet `
  --mimic-cxr-dir "D:\physionet\mimic-cxr-reports" `
  --ecg-index-report data/cases/local_case_id/artifacts/local_case_id.ecg-index.local.json `
  --skip-lab-probe `
  --output data/cases/local_case_id/artifacts/local_case_id.source-probe.local.json `
  --supplemental-output data/cases/local_case_id/artifacts/local_case_id.results.local.candidates.json
```
...
python -m backend.cases.source_refresh data/restricted/mietic_mimic_main_ed_enriched_cases.restricted.json `
  ...
  --ecg-index-report data/cases/local_case_id/artifacts/local_case_id.ecg-index.local.json `
  --output data/cases/local_case_id/prepared_case.json `
  --report-output data/cases/local_case_id/artifacts/local_case_id.source-refresh.local.json
```
...
```powershell
python -m backend.cases.source_acquisition data/restricted/local_case_id.source-refresh.local.json `
  --output data/restricted/local_case_id.source-acquisition.local.json
```
...
python -m backend.cases.source_acquisition_preflight data/restricted/mietic_mimic_main_ed_enriched_cases.restricted.json `
  ...
  --source-acquisition-report data/cases/local_case_id/artifacts/local_case_id.source-acquisition.local.json `
  --supplemental-results data/cases/local_case_id/artifacts/local_case_id.results.local.json `
  --output data/cases/local_case_id/artifacts/local_case_id.source-acquisition-preflight.local.json
```
...
```powershell
python -m backend.cases.hidden_wall data/cases/local_case_id/prepared_case.json `
  --output data/cases/local_case_id/artifacts/local_case_id.hidden-wall.json `
  --payload-output data/cases/local_case_id/artifacts/local_case_id.hidden-wall.payload.json
```
...
```powershell
python -m backend.cases.live_state_audit data/cases/local_case_id/prepared_case.json `
  --output data/cases/local_case_id/artifacts/local_case_id.live-state.json
```
...
```powershell
python -m backend.cases.release_gate_audit data/cases/local_case_id/prepared_case.json `
  --output data/cases/local_case_id/artifacts/local_case_id.release-gate.json
```
...
```powershell
python -m backend.cases.readiness data/cases/local_case_id/prepared_case.json `
  --require-playthrough `
  --playthrough-script data/cases/local_case_id/artifacts/local_case_id.playthrough.local.json
```
...
```powershell
python -m backend.cases.finalize data/cases/local_case_id/prepared_case.json `
  --review data/cases/local_case_id/artifacts/local_case_id.review.local.json `
  --playthrough-script data/cases/local_case_id/artifacts/local_case_id.playthrough.local.json `
  --output data/cases/local_case_id/learner-ready.local.json `
  --report-output data/cases/local_case_id/artifacts/local_case_id.finalization.json
```
...
```powershell
python -m backend.cases.review data/cases/local_case_id/prepared_case.json `
  --review data/cases/local_case_id/artifacts/local_case_id.review.local.json `
  --output data/cases/local_case_id/prepared_case.json
```
...
```powershell
python -m backend.cases.trajectory_review data/cases/local_case_id/prepared_case.json `
  --output data/cases/local_case_id/artifacts/local_case_id.trajectory-review.json `
  --review-template-output data/cases/local_case_id/artifacts/local_case_id.trajectory-review.local.json
```
...
```powershell
python -m backend.cases.playthrough data/cases/local_case_id/prepared_case.json `
  --script data/cases/local_case_id/artifacts/local_case_id.playthrough.local.json `
  --output data/cases/local_case_id/artifacts/local_case_id.playthrough-report.json `
  --package-output data/cases/local_case_id/artifacts/local_case_id.package.local.json
```
...
```powershell
python -m backend.cases.playthrough_review data/cases/local_case_id/prepared_case.json `
  --script data/cases/local_case_id/artifacts/local_case_id.playthrough.local.json `
  --output data/cases/local_case_id/artifacts/local_case_id.playthrough-review.json `
  --review-template-output data/cases/local_case_id/artifacts/local_case_id.playthrough-review.local.json
```
